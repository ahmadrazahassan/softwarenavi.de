...
const TOPICS = ["Vergleiche", "Testberichte", "Fristen & Recht", "Preisänderungen"];
    ...
    <div className="container-site pt-8">
      ...
      <div className="mt-6 grid gap-12 lg:grid-cols-[1.1fr_1fr] lg:gap-16">
        <div className="max-w-xl">
          <h1 className="font-heading text-[2rem] font-medium leading-[1.05] tracking-[-0.03em] sm:text-4xl md:text-5xl">Der Softwarenavi-Newsletter</h1>
          <p className="mt-4 text-lg leading-relaxed text-muted-foreground">
            Für Geschäftsführung, Buchhaltung und Personalabteilung: Was sich bei Unternehmenssoftware in Deutschland ändert, kurz gefasst und ohne
            Werbegetöse.
          </p>

          <ul className="mt-2 flex flex-wrap gap-2" aria-label="Themen">
            {TOPICS.map((t) => (
              <li key={t} className="inline-flex items-center rounded-md border border-border px-2.5 py-1 text-xs font-medium text-ink">
                {t}
              </li>
            ))}
          </ul>
          ...
          <ul className="mt-10 flex flex-col gap-6">
            {BENEFITS.map(({ icon: Icon, t, d }) => (
              <li key={t} className="flex gap-4">
                <span className="grid size-11 shrink-0 place-items-center rounded-lg bg-brand-light text-brand-dark" aria-hidden="true">
        ...
        <div className="rounded-xl border border-border bg-white p-6 shadow-[0_24px_60px_-30px_rgba(0,0,0,0.3)] md:p-8 lg:self-start">
          ...
          <NewsletterForm variant="full" source="newsletter-seite" />
        </div>
      </div>
    </div>
  );
